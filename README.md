...
<head>
  ...
  <style>
    ...
    #game-root{max-width:1200px;margin:20px auto;display:grid;grid-template-columns:1fr 340px;gap:18px}
    ...
    .buff-list{font-size:13px;color:var(--muted);margin:4px 0}
    ...
    .danger-badge{background:#fff0f0;border:1px solid #ffb3b3;color:#a40000}
...
<body>
  <div id="game-root">
    <header>
      <h1>カードバトルゲーム（UI改善版）</h1>
      <div class="small">・攻撃は常にエネルギー1消費・スキルはカードごとの cost を消費します</div>
    </header>
    ...
    <aside class="panel" id="detail-panel">
      ...
      <div id="detail" style="display:none">
        <div class="stat" id="d-name"></div>
        <div class="stat">体力: <span id="d-hp"></span></div>
        <div class="stat">攻撃: <span id="d-atk"></span>　防御: <span id="d-def"></span></div>
        <div class="stat">回復: <span id="d-heal"></span></div>
        <div class="stat">スキル: <span id="d-skill"></span>　<small id="d-cost"></small></div>
        <div class="stat">攻撃対象: <span id="d-attackTarget"></span></div>
        <div class="stat">特性:
          <span id="d-specials"></span>
        </div>
        <div class="stat">永続バフ:</div>
        <div id="d-perm" class="buff-list"></div>
        <div class="stat">一時バフ（残りターン）:</div>
        <div id="d-temp" class="buff-list"></div>
        <div class="stat">永続デバフ（上書き）:</div>
        <div id="d-perm-debuff" class="buff-list"></div>
      </div>
    </aside>
  </div>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCardDisplay(card) {
      ...
      const specials = JSON.parse(card.dataset._special || '[]').join(',');
      const usedText = card.dataset._skillUsed === 'true' ? '🔒スキル済' : '';
      const attackedText = card.dataset._attacked === 'true' ? '⚔攻撃済' : '';
      const tempList = JSON.parse(card.dataset._buffsTempList || '[]');
...
${attackedText}`.trim();
    ...
    function specialToJapanese(s) {
      ...
      if (s === 'doubleEnergyRegen') return 'エネ回復2倍';
      if (s === 'taunt') return '挑発（敵のターゲット誘導）';
    ...
    function renderDetail(card) {
      ...
      dSkill.innerText = card.dataset.skill || '-';
      dCost.innerText = `スキルコスト: ${card.dataset.cost || 0}`;
      dAttackTarget.innerText = card.dataset._attackTarget || '-';
      // specials
      const specials = JSON.parse(card.dataset._special || '[]');
      dSpecials.innerHTML = specials.length ? specials.map(s=>`<span class="badge special-badge">${specialToJapanese(s)}</span>`).join('') : '-';
      // perm
      const perm = JSON.parse(card.dataset._buffsPermanent || '{"hp":1,"atk":1,"def":1,"heal":1}');
      dPerm.innerHTML = `体力×${perm.hp}, 攻撃×${perm.atk}, 防御×${perm.def}, 回復×${perm.heal}`;
      // temp list
      const temp = JSON.parse(card.dataset._buffsTempList || '[]');
      if (temp.length===0) dTemp.innerText='-'; else {
      ...
      dPermDebuff.innerText = card.dataset._permanentDebuff ? JSON.stringify(JSON.parse(card.dataset._permanentDebuff)) : '-';
    }

    // ========== ここからはロジック（攻撃/スキル/バフ等） ==========
    function applyDamage(target, atkRaw, options = {}) {
      ...
      // if displayed detail is this card, refresh detail
      if (detailEl.style.display!=='none' && (detailEl && (document.querySelector(`[data-id="${target.dataset.id}"]`) === target))) renderDetail(target);
    }
    ...
    function applyBuffToCard(targetCard, buffs, duration, permanent, options = {}) {
      ...
      if (specials.includes('grantExtraAction')) {
        delete targetCard.dataset._attacked;
      }
      if (specials.includes('taunt') && duration>0) {
        targetCard.dataset._tauntRemaining = String(Math.max(parseInt(targetCard.dataset._tauntRemaining || '0',10), duration));
      }
      updateCardDisplay(targetCard);
      if (detailEl.style.display!=='none' && (detailEl && (document.querySelector(`[data-id="${targetCard.dataset.id}"]`) === targetCard))) renderDetail(targetCard);
    }
    ...
    function enemyTurn() {
      ...
      enemies.forEach(enemy => {
        ...
        // attack fallback (respect attackTarget and taunt)
